...
const Decoder = require("./vendor/Broadway1/Decoder");

// Your existing code remains unchanged
const canvas = document.getElementById("canvas");
const decoder = new Decoder();

const websocket = new WebSocket("ws://localhost:3000"); // Use the correct WebSocket server address

websocket.addEventListener("message", (event) => {
  const receivedData = new Uint8Array(event.data);

  // Decode and display the H.264 video frames using Broadway.js
  decoder.decode(receivedData);
  const image = decoder.getImageData();
  if (image !== null) {
...
},{"./vendor/Broadway1/Decoder":3}],3:[function(require,module,exports){
